...
func TestLoadKeyValues_SnapshotReferenceDetection(t *testing.T) {
	// Verify that snapshot reference settings are detected, excluded from kvSettings,
	// and the tracing option is set
	mockClient := &mockSettingsClient{}

	regularValue := "regular"
	snapshotRefValue := `{"snapshot_name":"my-snapshot"}`
	...
	settings := []azappconfig.Setting{
		...
		{
			Key:         toPtr("snapshot-ref-key"),
			Value:       &snapshotRefValue,
			ContentType: toPtr(snapshotReferenceContentType),
		},
	}
	...
	// Regular key should be in settings
	assert.Equal(t, &regularValue, azappcfg.keyValues["regular-key"])
	// Snapshot reference key should NOT be in settings
	assert.NotContains(t, azappcfg.keyValues, "snapshot-ref-key")
	// Tracing should be set
	assert.True(t, azappcfg.tracingOptions.UseSnapshotReference)
